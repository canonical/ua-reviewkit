...
class TaskBag(object):
    def __init__(self):
        ...
        signal_list = [signal.SIGABRT, signal.SIGTERM, signal.SIGINT]
        for sig in signal_list:
            signal.signal(sig, self.abort_cleanup)

    def abort_cleanup(self, signum, frame):
        LOG.debug("Received signal %s: Frame: %s", signum, frame)
        LOG.info("User aborted! Cleaning up tasks")
        for task in self.started_tasks + self.uncompleted_tasks:
            LOG.info(".")
            task.abort()
        LOG.info("Bye!")
...
class CollectorTask(object):
    ...
    def abort(self):
        self.status = "ABORTED"
        self.print_status(msg="Aborting task")
        self.executor.abort_run()
...
def worker(task_bag, _):
    ...
    while True:
        with THREAD_LOCK:
            if task_bag.task_queue.empty():
                LOG.debug("Thread %s exiting", thread_name)
                break
            collector = task_bag.task_queue.get()
        ...
        with THREAD_LOCK:
            task_bag.started_tasks.append(collector)
            task_bag.uncompleted_tasks.append(collector)
        ...
        collector.teardown()
        with THREAD_LOCK:
            task_bag.completed_tasks.append(collector)
            task_bag.uncompleted_tasks.remove(collector)
            task_bag.task_queue.task_done()

        with THREAD_LOCK:
            started_tasks_ids = ",".join(
                [tid.machine[0] for tid in task_bag.started_tasks])
            uncompleted_tasks_ids = ",".join(
                [tid.machine[0] for tid in task_bag.uncompleted_tasks])
            completed_task_ids = ",".join(
                [tid.machine[0] for tid in task_bag.completed_tasks])
            LOG.info(
                "Finished collection for %s, machine: %s. %s targets "
                "unfinished.",
                collector.app,
                collector.machine[0],
                len(task_bag.uncompleted_tasks))
